    ...
    for level in level_to_probs:
        ...
        for seq_idx, decode_out in enumerate(decode_outs):
            ...
            if len(decode_out.ids) != len(chunked_phonemes_batch[seq_idx]) and (
                ...
                # Guard: only assign if shapes are compatible
                non_missing_count = (ref_aligned_ids != missing_placeholder).sum().item()
                mask_count = mask.sum().item()
                if non_missing_count > 0 and mask_count > 0 and len(probs) >= mask_count:
                ...
                new_probs = new_probs[phonemes_mask] if len(new_probs) >= len(phonemes_mask) else torch.zeros(len(aligned_ids), dtype=torch.float32)
