...
class Muaalem:
    def __init__(
        ...
        self.model = Wav2Vec2BertForMultilevelCTC.from_pretrained(model_name_or_path, local_files_only=True)
        self.multi_level_tokenizer = MultiLevelTokenizer(model_name_or_path, local_files_only=True)
        self.processor = AutoFeatureExtractor.from_pretrained(model_name_or_path, local_files_only=True)
