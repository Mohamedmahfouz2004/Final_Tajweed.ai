...
class AlignmentConfig:
    ...
    def from_config(cls, cfg):
        ...
        for attr in ('ALPHA', 'BETA', 'SEGMENT_THRESHOLD', 'WORD_THRESHOLD',
                     ...
                     'SEGMENT_STRIDE'):
            if hasattr(cfg, attr):
                setattr(inst, attr, getattr(cfg, attr))
        return inst
...
class SegmentGenerator:
    ...
    def generate_tracking_candidates(self, anchor_pos: int, sura: Optional[int] = None) -> List[SegmentCandidate]:
        ...
        # Constrain window to surah boundaries so we never drift into adjacent surahs
        if sura is not None:
...
class SegmentScorer:
    ...
    def find_best(self, spoken_text: str, candidates: List[SegmentCandidate], top_n: int = 5) -> List[SegmentCandidate]:
        if not candidates:
            return []
        # Use rapidfuzz batch scoring (C extension) instead of Python loop — ~5-10x faster
        ...
        if not results:
            return []
        # Map results back to candidate objects
        best = []
...
class SpecialPhraseHandler:
    ...
    def strip(self, spoken_words: List[str]) -> Tuple[List[str], List[str]]:
        """Return (remaining_words, detected_phrase_names)."""
        detected: List[str] = []
...
class QuranAlignmentEngine:
    ...
    def align(self, transcript: str, anchor_pos: int, mode: str,
        ...
        # Strip Istiatha / Basmallah so they don't misalign the first aya
        remaining, detected = self.phrase_handler.strip(spoken_words)
        if detected:
            logger.info(f"Stripped special phrases: {detected}")
        ...
        # Confidence = average similarity of matched words (more granular than ratio)
    ...
    def get_page_verse_ids(self, sura: int) -> List[int]:
        """Return all verse IDs for a given surah (used as 'page')"""
        return [v.aya_id for v in self.verses if v.sura == sura]

    def get_surah_first_index(self, sura: int) -> int:
        """Return the global word index of the first word in the given surah"""
    ...
    def get_surah_words(self, sura: int) -> List[Dict]:
        """Return all words for display in the frontend"""
        result = []
